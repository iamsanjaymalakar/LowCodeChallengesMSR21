...
df.to_csv('Questions_removed_wrong_tags.csv', index=False)
print('After', len(df))

print('\nREMOVING QUESTIONS WITH NEGATIVE SCORES')
print('Before', len(df))
df = df[df['Score'] >= 0]
df.to_csv('Questions_removed_negative_scores.csv', index=False)
print('After', len(df))

print('\nREMOVING DUPLICATE SCORES')
print('Before', len(df))
df.drop_duplicates('Id',inplace=True)
df.to_csv('Questions_removed_duplicates.csv', index=False)
print('After', len(df))
